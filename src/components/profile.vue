<template>
    ...
    <section v-if="loadingStatus === 'notloading'" class=" mt-10 bg-gradient-to-r from-green-lime via-light-dark to-dark-house">
        <div class="flex justify-center">
            <div class="grid md:flex md:py-6 py-4">
                ...
                <div class="bg-orange-300 w-[18rem] rounded-lg shadow-md shadow-black h-[18rem] md:w-[20rem] md:h-[18rem] md:mr-3 flex justify-center items-center">
    ...
    <section v-if="loadingStatus === 'notloading'" class="mb-20  bg-gradient-to-r from-green-lime via-light-dark to-dark-house">
        <div class="pl-20 flex justify-center">
            <div v-if="edittoggle && zoekProfiel" class="py-6 grid text-black md:ml-10">
                <p class="bg-yellow-500 w-[75%] mb-2 p-1 rounded-lg font-serif font-bold">Hallo {{ zoekProfiel.naam }}, vul alles opnieuw in met de juiste gegevens.</p>
                    <p class="text-white md:text-lg font-serif"> <i class="fa-solid fa-signature fa-sm" style="color: #ffffff;"></i> Naam:   {{ zoekProfiel.naam }}  </p><input    class="p-1 w-[75%] rounded-md active:border-white" type="text" v-model="inputNaam" placeholder="Naam" required>
                    <p class="text-white md:text-lg font-serif"> <i class="fa-solid fa-house fa-sm" style="color: #f2f2f2;"></i> Tussenvoegsel:  {{ zoekProfiel.tussenvoegsel }}   </p><input  class="p-1 w-[75%] rounded-md active:border-white" type="text" v-model="inputTussenvoegsel" placeholder="Tussenvoegsel" required>
                    <p class="text-white md:text-lg font-serif"> <i class="fa-solid fa-house fa-sm" style="color: #f2f2f2;"></i> Achternaam:   {{ zoekProfiel.achternaam }}  </p><input  class="p-1 w-[75%] rounded-md active:border-white" type="text" v-model="inputAchternaam" placeholder="Achternaam" required>
                    <p class="text-white md:text-lg font-serif"> <i class="fa-solid fa-cake-candles" style="color: #d1d7e0;"></i> Geboortedatum:  {{ zoekProfiel.gboortedatum }} </p><input class="p-1 w-[75%] rounded-md active:border-white" type="date" v-model="inputGbDatum" placeholder="" required>
                    <p class="text-white md:text-lg font-serif"> <i class="fa-solid fa-house fa-sm" style="color: #f2f2f2;"></i> Woonplaats: {{ zoekProfiel.woonplaats }}    </p><input  class="p-1 w-[75%] rounded-md active:border-white" type="text" v-model="inputWoonplaats" placeholder="Woonplaats" required>
                    <p class="text-white md:text-lg font-serif"> <i class="fa-solid fa-location-dot fa-sm" style="color: #fafafa;"></i> Adres:  {{ zoekProfiel.adres }} </p><input  class="p-1 w-[75%] rounded-md active:border-white" type="text" v-model="inputAdres" placeholder="Adres" required>
                    <p class="text-white md:text-lg font-serif"> <i class="fa-solid fa-briefcase fa-sm" style="color: #ffffff;"></i> Functie:  {{ zoekProfiel.functie }} </p><input   class="p-1 w-[75%] rounded-md active:border-white" type="text" v-model="inputFunctie" placeholder="Functie" required>
                    ...
                    <input @click="editProfile(zoekProfiel.id)" class="mt-4 w-[75%] bg-white py-2 px-3 rounded-md active:bg-light-dark active:text-white shadow-md shadow-green-lime" type="submit" value="Bewerk">
            </div>
        </div>
    </section>
